...
TMF is a framework for modeling multimedia system programs with a dataflow model. A dataflow model is a directed graph in which nodes represent computation tasks (called actors) and edges represent communication between a pair of actors. Modeling multimedia system programs with dataflow allows programs to be parallelized and distributed. TMF implements some OpenCV actors.
